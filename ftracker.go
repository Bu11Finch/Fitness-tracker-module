...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(averageSpeedMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	swimmingAverageSpeedKmh := swimmingMeanSpeed(lengthPool, countPool, duration)
	calories := ((swimmingAverageSpeedKmh + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration)
	return calories
